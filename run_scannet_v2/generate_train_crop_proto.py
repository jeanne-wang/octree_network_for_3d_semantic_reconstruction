...
def Input3D(name, tops, batch_size, data_list_file, groundtruth_list_file,
    preload_data, row_major, height, width, depth, num_classes, customized_crop, phase):
    ...
    layer.input3d_param.preload_data = preload_data
    layer.input3d_param.row_major = row_major
    ...
    layer.input3d_param.customized_crop = customized_crop
...
def OctSegLossPrep(name, bottoms, input_key_layer, ref_key_layer):
    layer = caffe_pb2.LayerParameter()
...
def OctPrimalDualUpdateFull(init_variable_bottoms, init_variable_key_layer, datacost_bottom, datacost_key_layer, 
                                ...
                                gt_bottom, gt_key_layer, nlevels, num_iters):
    ...
    for level in range(nlevels):
        if level > 0:
            ...
            layers.append(PropProbConv('prop_prob{}'.format(level-1), prop_prob_conv_bottoms, decoding_filter_size, decoding_w_std, decoding_lr_mult))
        ...
        layers.append(OctSegLossPrep('seg_gt{}'.format(level), [gt_bottom, 'seg_prob{}'.format(level)], gt_key_layer, variable_key_layer))
        

    return layers


def SegCrossEntropyLoss(name, bottoms, clip_epsilon, unknown_label):
    ## bottom[0] gt bottom[1] pred
    layer = caffe_pb2.LayerParameter()
    layer.name = name
    layer.type = 'OctPrimalDualWeightedCrossEntropyLoss'
    layer.primal_dual_weighted_cross_entropy_loss_param.unknown_label = unknown_label
    layer.primal_dual_weighted_cross_entropy_loss_param.clip_epsilon = clip_epsilon
    layer.bottom.extend(bottoms)
    layer.top.extend([name])
...
def SegLossBlock(nlevels, seg_prob_bottoms, seg_gt_bottoms, clip_epsilon, unknown_label):
    layers = []
    for level in range(nlevels):
        bottoms = []
        ...
        layers.append(SegCrossEntropyLoss('seg_loss_l{}'.format(level), bottoms, clip_epsilon, unknown_label))
    return layers
...
def OctReconstructionAccuracy(name, tops, bottoms, gt_key_layer, pr_key_layer, max_level, freespace_label, unknown_label):
    layer = caffe_pb2.LayerParameter()
    layer.name = name
    layer.type = 'OctAccuracy3DReconstruction'
    layer.bottom.extend(bottoms)
    layer.top.extend(tops)
...
def create_model(batch_size, val_batch_size, num_classes, train_data_list_file, train_groundtruth_list_file, 
                val_data_list_file, val_groundtruth_list_file, preload_data, row_major, customized_crop,
                max_level, nlevels, niter_l0, niter_l1, niter_l2, sigma, tau, lam, softmax_scale, 
                clip_epsilon, freespace_label, unknown_label):
    ...
    nbh_size = 3
    # train
    layers.append(Input3D('input', ['dense_datacost', 'dense_gt'], batch_size, 
                        train_data_list_file, train_groundtruth_list_file,
                        preload_data, row_major, input_size, input_size, input_size, num_classes, 
                        customized_crop, 'train'))
    ...
    # validation
    layers.append(Input3D('input', ['dense_datacost', 'dense_gt'], val_batch_size, 
                        val_data_list_file, val_groundtruth_list_file,
                        preload_data, row_major, input_size, input_size, input_size, num_classes, 
                        customized_crop, 'test'))
    ...
    layers.append(OctConvert('input_gt', 'gt', 'dense_gt', min_level, True))
    ...
    layers.extend(SegLossBlock(nlevels, seg_probs, seg_gts, clip_epsilon, unknown_label))
    ...
    layers.append(OctReconstructionAccuracy('accuracy', ['freespace_accuracy', 'semantic_accuracy', 'overall_accuracy'], ['gt', 'pred'],
        'input_gt', 'output_pred', max_level, freespace_label, unknown_label))
...
def main(args):
    model = create_model(args.batch_size, args.val_batch_size, args.num_classes, args.train_data_list_file, args.train_groundtruth_list_file, 
                        args.val_data_list_file, args.val_groundtruth_list_file, args.preload_data, args.row_major, args.customized_crop,
                        args.max_level, args.nlevels, args.niter_l0, args.niter_l1, args.niter_l2, args.sig, args.tau, args.lam, args.softmax_scale,
                        args.clip_epsilon, args.freespace_label, args.unknown_label)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--row_major", type=bool, default=True)
    parser.add_argument("--customized_crop", type=bool, default=False)
    ...
    parser.add_argument("--clip_epsilon", type=float, default=10e-8)
