...
def main():
    ...
    # unobserved = (np.max(volume, axis=-1)<0.5)
    pred = np.argmax(volume, axis=-1)
    # pred[unobserved] = args.freespace_label

    if os.path.exists(os.path.join(test_scene, 'pred.npy')):
        os.remove(os.path.join(test_scene, 'pred.npy'))

    np.save(os.path.join(test_scene, 'pred.npy'), pred)
