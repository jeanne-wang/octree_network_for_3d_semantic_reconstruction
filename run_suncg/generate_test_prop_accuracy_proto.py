...
def OctPrimalDualUpdateFull(init_variable_bottoms, init_variable_key_layer, datacost_bottom, datacost_key_layer, 
                                ...
                                gt_key_layer, nlevels, num_iters):
...
def BinaryClassificationAccuracy(name, tops, bottoms):
    layer = caffe_pb2.LayerParameter()
    layer.name = name
    layer.type = 'BinaryClassificationAccuracy'
    layer.bottom.extend(bottoms)
    layer.top.extend(tops)

    return layer

def PropAccuracyBlock(nlevels, prop_prob_bottoms, prop_gt_bottoms):
    layers = []
    for level in range(nlevels-1):
        ...
        tops = []
        tops.append('prop_overall_accuracy_l{}'.format(level))
        tops.append('prop_positive_accuracy_l{}'.format(level))
        tops.append('prop_negative_accuracy_l{}'.format(level))
        layers.append(BinaryClassificationAccuracy('prop_accuracy_l{}'.format(level),tops, bottoms))

    return layers
        
def Silence(name, bottom):
    layer = caffe_pb2.LayerParameter()
    layer.name = name
    layer.type = 'Silence'
    layer.bottom.extend([bottom]) 
...
def create_model(batch_size, num_classes, test_data_list_file, test_groundtruth_list_file, preload_data, 
    ...
    layers.extend(OctPrimalDualUpdateFull(['u', 'u_', 'm', 'l', 'num', 'nbh', 'nbh_of'], 'init_variable', 'datacost_lam', 'input_datacost', max_level, num_classes, 
                                        ...
                                        update_lr_mult, update_w_std, update_filter_size, tau, sigma, 
                                        decoding_filter_size, decoding_w_std, decoding_lr_mult, softmax_scale, 
                                        'input_gt', nlevels, num_iters))

    ## prop accuracy
    prop_probs = []
    prop_gts = []
    for level in range(nlevels-1):
        prop_probs.append('prop_prob{}_sig'.format(level))
        prop_gts.append('prop_gt{}'.format(level))

    layers.extend(PropAccuracyBlock(nlevels, prop_probs, prop_gts))

    ## silence
    layers.append(Silence('gt_silence', 'gt'))
    for level in range(nlevels):
        layers.append(Silence('seg_prob{}_silence'.format(level), 'seg_prob{}'.format(level)))
