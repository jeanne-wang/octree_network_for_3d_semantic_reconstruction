    ...
    GeneralVoxelGrid<VALUE> to_voxel_grid(){
        ...
        for(typename std::tr1::unordered_map<KEY, VALUE>::iterator iter = _hash_table.begin(); iter != _hash_table.end(); ++iter){
            ...
            for(int i=0; i < len; i++){
                for(int j=0; j < len; j++){
                    for(int k=0; k < len; k++){
                        ret.set_element(x + i, y + j, z + k, iter->second);
                    }
                }
            }
                ...
                for(unsigned int j=0; j<vg.width(); j+=step){
                    for(unsigned int k=0; k<vg.depth(); k+=step){
                        ...
                        if(compute_level(key) == level){
                            ...
                            VALUE val = values_arr[i*dim*dim + j*dim +k];
                            int count = 0;
                            for(int ii=0; ii<2; ii++){
                                for(int jj=0; jj<2; jj++){
                                    for(int kk=0; kk<2; kk++){
                                        ...
                                        if(comp_lev == level){
                                            VALUE comp_val = values_arr[(i+ii*step/2)*dim*dim + (j+jj*step/2)*dim + k+kk*step/2];
                                            if(comp_val == val) count++;
                                        }
                                    }
                                }
                            }
