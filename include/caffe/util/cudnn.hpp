...
// add by xiaojuan
template <typename Dtype>
inline void createTensor5dDesc(cudnnTensorDescriptor_t* desc) {
  CUDNN_CHECK(cudnnCreateTensorDescriptor(desc));
}
...
// add by xiaojuan
template <typename Dtype>
inline void setTensor5dDesc(cudnnTensorDescriptor_t* desc, 
    int n, int c, int h, int w, int d,
    int stride_n, int stride_c, int stride_h, int stride_w, int stride_d){

  const int dimA[5] = {n, c, h, w, d};
  const int strideA[5] = {stride_n, stride_c, stride_h, stride_w, stride_d};
...
inline void setTensor5dDesc(cudnnTensorDescriptor_t* desc, 
  ...
  int stride_d = 1;
  int stride_w = d * stride_d;
  int stride_h = w * stride_w;
  int stride_c = h * stride_h;
  int stride_n = c * stride_c;
  setTensor5dDesc<Dtype>(desc, n, c, h, w, d, stride_n, stride_c, stride_h, stride_w, stride_d);
}
...
// add by xiaojuan
template <typename Dtype>
inline void createFilter5dDesc(cudnnFilterDescriptor_t* desc,
    int n, int c, int h, int w, int d) {
  CUDNN_CHECK(cudnnCreateFilterDescriptor(desc));
  const int filterDimA[5] = {n, c, h, w, d};
...
// add by xiaojuan
template <typename Dtype>
inline void setConvolution3dDesc(cudnnConvolutionDescriptor_t* conv,
    cudnnTensorDescriptor_t bottom, cudnnFilterDescriptor_t filter,
    int pad_h, int pad_w, int pad_d, int stride_h, int stride_w, int stride_d) {

  const int padA[3] = {pad_h, pad_w, pad_d};
  const int strideA[3] = {stride_h, stride_w, stride_d};
  const int dilationA[3] = {1, 1, 1};
  CUDNN_CHECK(cudnnSetConvolutionNdDescriptor(*conv,
      3, padA, strideA, dilationA, CUDNN_CROSS_CORRELATION,
      dataType<Dtype>::type));
