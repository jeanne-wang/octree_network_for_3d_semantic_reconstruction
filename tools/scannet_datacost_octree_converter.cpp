...
int main(int argc, char* argv[]) {
    if ( !register_cmd_options(argc, argv) ) {
        ...
        std::string output_file_path = getFilePath(output_file);
        ...
        if(input_ext == "dat"){
            ...
            // write scene size info
            std::string size_info_file = output_file_path + "/info.txt";
            std::ofstream info(size_info_file.c_str());
            info<<height<<" "<<width<<" "<<depth<<"\n";
            ...
            for (int x = 0; x < num_h; x++){
                for(int y = 0; y < num_w; y++){
                    ...
                    vg.read_datacost(input_file, num_classes, crop);
                    ...
                    std::string info_file = output_file_path + "/info" + num2string(count) + ".txt";
                    std::ofstream out_info(info_file.c_str());
                    out_info<<crop[0].first<<" "<<crop[0].second<<"\n";
                    out_info<<crop[1].first<<" "<<crop[1].second<<"\n";
                    out_info<<crop[2].first<<" "<<crop[2].second<<"\n";
                    count++;
