...
void my_caffe_conv(const Blob<Dtype>* in, 
...
template void my_caffe_conv(const Blob<float>* in,
    const vector<shared_ptr<Blob<float> > >& weights,
    Blob<float>* out);
template void my_caffe_conv(const Blob<double>* in,
...
#ifdef USE_CUDNN
template <typename Dtype>
class CuDNNDualUpdateLayerTest : public GPUDeviceTest<Dtype>{
 protected:
  CuDNNDualUpdateLayerTest()
  ...
  virtual ~CuDNNDualUpdateLayerTest() {
    delete blob_bottom_;
...
TYPED_TEST_CASE(CuDNNDualUpdateLayerTest, TestDtypes);

TYPED_TEST(CuDNNDualUpdateLayerTest, TestSetup) {
  LayerParameter layer_param;
  DualUpdateParameter* dual_update_param =
      layer_param.mutable_dual_update_param();

  dual_update_param->add_kernel_size(3);
  dual_update_param->add_start_pad(1);
  ...
  shared_ptr<Layer<TypeParam> > layer(
      new CuDNNDualUpdateLayer<TypeParam>(layer_param));
...
/*TYPED_TEST(CuDNNDualUpdateLayerTest, TestConvolution) {
  LayerParameter layer_param;
  DualUpdateParameter* dual_update_param =
      layer_param.mutable_dual_update_param();
  ...
  dual_update_param->set_num_output(6);
  dual_update_param->mutable_weight_filler()->set_type("gaussian");
  //dual_update_param->mutable_weight_filler()->set_value(1.);
  dual_update_param->set_sigma(0.01);
  ...
  my_caffe_conv(this->blob_bottom_2_, layer->blobs(),
...
TYPED_TEST(CuDNNDualUpdateLayerTest, TestGradient) {
  LayerParameter layer_param;
  DualUpdateParameter* dual_update_param =
      layer_param.mutable_dual_update_param();
  ...
  dual_update_param->mutable_weight_filler()->set_type("gaussian");
  dual_update_param->set_sigma(0.01);
  CuDNNDualUpdateLayer<TypeParam> layer(layer_param);
