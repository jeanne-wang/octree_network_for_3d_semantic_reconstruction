...
TYPED_TEST(NeuronLayerTest, TestPrimalProjection) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  PrimalProjectionLayer<Dtype> layer(layer_param);
  layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);
  layer.Forward(this->blob_bottom_vec_, this->blob_top_vec_);
  // Now, check values
  const Dtype* bottom_data = this->blob_bottom_->cpu_data();
  ...
  for (int i = 0; i < this->blob_bottom_->count(); ++i) {
    EXPECT_GE(top_data[i], 0.);
    EXPECT_LE(top_data[i], 1.);
    EXPECT_TRUE(top_data[i] == 0 || top_data[i] == bottom_data[i] || top_data[i] == 1 );
  }
}

/*TYPED_TEST(NeuronLayerTest, TestPrimalProjectionGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  PrimalProjectionLayer<Dtype> layer(layer_param);
