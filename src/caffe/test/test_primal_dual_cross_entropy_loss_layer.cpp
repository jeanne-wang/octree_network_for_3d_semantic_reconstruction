...
class PrimalDualLossLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  PrimalDualLossLayerTest()
      ...
      for(int i = 0; i < 4; i++){
        for(int j = 0; j < 4; j++){
          for(int k = 0; k < 4; k++){
            ...
            gt_data[(((n * 3 + label) * 4 + i) * 4 + j) * 4 + k] = 1;
          }
        }
      }
    }
  ...
  virtual ~PrimalDualLossLayerTest() {
...
TYPED_TEST_CASE(PrimalDualLossLayerTest, TestDtypesAndDevices);

TYPED_TEST(PrimalDualLossLayerTest, TestLoss) {
  typedef typename TypeParam::Dtype Dtype;
