...
class OctDualUpdateLayerConvTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  OctDualUpdateLayerConvTest()
  ...
  virtual ~OctDualUpdateLayerConvTest() {
    delete blob_bottom_;
...
TYPED_TEST_CASE(OctDualUpdateLayerConvTest, TestDtypesAndDevices);

TYPED_TEST(OctDualUpdateLayerConvTest, TestSetup) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  OctDualUpdateParameter* dual_update_param =
      layer_param.mutable_oct_dual_update_param();

  dual_update_param->set_filter_size(2);
  dual_update_param->set_output_channels(6);
  ...
  shared_ptr<Layer<Dtype> > layer(
      new OctDualUpdateLayer<Dtype>(layer_param));
...
TYPED_TEST(OctDualUpdateLayerConvTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  OctDualUpdateParameter* dual_update_param =
      layer_param.mutable_oct_dual_update_param();
  ...
  OctDualUpdateLayer<Dtype> layer(layer_param);
