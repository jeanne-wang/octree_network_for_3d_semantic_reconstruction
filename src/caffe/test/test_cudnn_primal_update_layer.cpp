...
class CuDNNPrimalUpdateLayerTest : public GPUDeviceTest<Dtype>{
 protected:
  CuDNNPrimalUpdateLayerTest()
  ...
  virtual ~CuDNNPrimalUpdateLayerTest() {
    delete blob_bottom_;
...
TYPED_TEST_CASE(CuDNNPrimalUpdateLayerTest, TestDtypes);

TYPED_TEST(CuDNNPrimalUpdateLayerTest, TestSetup) {
  LayerParameter layer_param;
  PrimalUpdateParameter* primal_update_param =
      layer_param.mutable_primal_update_param();

  primal_update_param->add_kernel_size(3);
  primal_update_param->add_start_pad(1);
  ...
  shared_ptr<Layer<TypeParam> > layer(
      new CuDNNPrimalUpdateLayer<TypeParam>(layer_param));
...
TYPED_TEST(CuDNNPrimalUpdateLayerTest, TestGradient) {
  LayerParameter layer_param;
  PrimalUpdateParameter* primal_update_param =
      layer_param.mutable_primal_update_param();
  ...
  CuDNNPrimalUpdateLayer<TypeParam> layer(layer_param);
