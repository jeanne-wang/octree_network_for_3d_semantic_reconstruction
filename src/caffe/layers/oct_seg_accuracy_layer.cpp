...
#include "caffe/layers/oct_seg_accuracy_layer.hpp"
...
void OctSegAccuracyLayer<Dtype>::LayerSetUp(
    ...
    has_ignore_label_ = this->layer_param_.oct_accuracy_3dreconstruction_param().has_ignore_label();
    if(has_ignore_label_){
        ignore_label_ = this->layer_param_.oct_accuracy_3dreconstruction_param().ignore_label();
    }
...
template <typename Dtype>
void OctSegAccuracyLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
    ...
    CHECK_EQ(bottom[0]->shape(0), batch_size_)
        << "gt and label must have the same num.";



    num_elements_true_ = bottom[0]->shape(1);
...
template <typename Dtype>
void OctSegAccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    ...
    const Dtype* labels_true_data = bottom[0]->cpu_data();
    ...
    for(int bt = 0; bt < batch_size_; bt++){
        ...
        for(GeneralOctree<int>::iterator it=gt_keys_octree.begin(); it!=gt_keys_octree.end(); it++){
            ...
            int label = static_cast<int>(labels_true_data[bt * num_elements_true_ + it->second]);
            ...
            if(label == freespace_label_){

                freespace_total_count += count;
                freespace_correct_count += match_count;
            }else if(label != ignore_label_ && label != unknown_label_){
...
template <typename Dtype>
void OctSegAccuracyLayer<Dtype>::arg_max(const Blob<Dtype>* input, int* output){
...
template <typename Dtype>
int OctSegAccuracyLayer<Dtype>::get_octree_cell_match_count(GeneralOctree<int> &pr_keys_octree, 
...
INSTANTIATE_CLASS(OctSegAccuracyLayer);
REGISTER_LAYER_CLASS(OctSegAccuracy);
