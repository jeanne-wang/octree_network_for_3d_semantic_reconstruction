...
void OctPrimalDualCrossEntropyLossLayer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void OctPrimalDualCrossEntropyLossLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  CHECK_EQ(bottom[0]->shape(0), bottom[1]->shape(0))
    << "The batch size of groundtruth data and the predicted probs must the the same.";
  CHECK_EQ(bottom[0]->shape(1), bottom[1]->shape(1))
    << "The number of channels of groundtruth data and the predicted probs must the the same.";
  CHECK_EQ(bottom[0]->shape(2), bottom[1]->shape(2))
    << "The number of voxels of groundtruth data and the predicted probs must the the same.";
...
template <typename Dtype>
void OctPrimalDualCrossEntropyLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  if(freespace_weighted_){
    ...
    for(int n = 0; n < num_; n++){
      for(int i = 0; i < num_pixels_; i++){
        ...
        Dtype max_val = gt_clip_data[n * num_classes_ * num_pixels_ + i];
        int max_ind = 0; // init the max_val and max_ind in the first channel
        Dtype sum = -1 * gt_log_pred_data[n * num_classes_ * num_pixels_ + i];
        for(int c = 1; c < num_classes_; c++){
          int ind = (n * num_classes_ + c) * num_pixels_ + i;
...
template <typename Dtype>
void OctPrimalDualCrossEntropyLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(OctPrimalDualCrossEntropyLossLayer);
#endif

INSTANTIATE_CLASS(OctPrimalDualCrossEntropyLossLayer);
REGISTER_LAYER_CLASS(OctPrimalDualCrossEntropyLoss);
