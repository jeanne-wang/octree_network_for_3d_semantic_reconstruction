#include "caffe/layers/oct_single_channel_input_layer.hpp"
...
void OctSingleChannelInputLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void OctSingleChannelInputLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top){
...
template <typename Dtype>
void OctSingleChannelInputLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
    ...
    for(int bt=0; bt< _batch_size; bt++)
    {
        ...
        int counter = 0;
        for(typename GeneralOctree<float>::iterator it=_batch_octrees[bt].begin(); it!=_batch_octrees[bt].end(); it++)
        {   
    
            top_values[bt * num_elements + counter] = (Dtype)it->second;
...
template <typename Dtype>
void OctSingleChannelInputLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
        LOG(FATAL) << "Backward not implemented";
}

template <typename Dtype>
int OctSingleChannelInputLayer<Dtype>::select_next_batch_models(vector<int> labels)
{
...
template <typename Dtype>
void OctSingleChannelInputLayer<Dtype>::load_data_from_disk(){
    ...
    while(infile >> name){
        ...
        if(_preload_data){
            // the ground truth label is in format value in suncg
            GeneralOctree<float> tree;
            tree.from_file(name);
...
INSTANTIATE_CLASS(OctSingleChannelInputLayer);
REGISTER_LAYER_CLASS(OctSingleChannelInput);
