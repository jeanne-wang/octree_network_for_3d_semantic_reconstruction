...
void CuDNNPrimalUpdateLayer<Dtype>::LayerSetUp(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  PrimalUpdateLayer<Dtype>::LayerSetUp(bottom, top);
  std::cout << "Cudnn PrimalUpdateLayer setting up...." << std::endl;
  ...
  cudnn::createFilter5dDesc<Dtype>(&filter_desc_,
      this->channels_, this->num_output_, 
...
template <typename Dtype>
void CuDNNPrimalUpdateLayer<Dtype>::Reshape(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {

  PrimalUpdateLayer<Dtype>::Reshape(bottom, top);
  CHECK_EQ(3, this->num_spatial_axes_)
      << "Dual input must have 3 spatial axes "
  ...
  cudnn::setConvolution3dDesc<Dtype>(&conv_desc_, top_desc_,
  ...
  if (fwd_algo_[0] == CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_PRECOMP_GEMM) {
    ...
    if (status != CUDNN_STATUS_SUCCESS || winograd_workspace_size >= workspace_limit_bytes) {
      fwd_algo_[0] = CUDNN_CONVOLUTION_FWD_ALGO_IMPLICIT_GEMM;
    } else {
      fwd_algo_[0] = CUDNN_CONVOLUTION_FWD_ALGO_WINOGRAD;
    }
  }
  ...
  CUDNN_CHECK(cudnnGetConvolutionBackwardFilterAlgorithm(handle_[0],
      top_desc_, bottom_desc_, conv_desc_, filter_desc_,
  ...
  CUDNN_CHECK(cudnnGetConvolutionBackwardDataAlgorithm(handle_[0],
      filter_desc_, bottom_desc_, conv_desc_, top_desc_,
  ...
  if (total_max_workspace > workspaceSizeInBytes) {
    ...
    if (err != cudaSuccess) {
      ...
      workspace_bwd_filter_sizes_[0] = 0;
      workspace_bwd_data_sizes_[0] = 0;
      fwd_algo_[0] = CUDNN_CONVOLUTION_FWD_ALGO_FFT_TILING;
...
template <typename Dtype>
CuDNNPrimalUpdateLayer<Dtype>::~CuDNNPrimalUpdateLayer() {
...
INSTANTIATE_CLASS(CuDNNPrimalUpdateLayer);
